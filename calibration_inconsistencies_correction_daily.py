...
class BiasCorrector_daily:
    ...
    def compute_bias_from_SeaWiFS(self, data_merged, data_SeaWiFS, lat):
        ...
        data_period = data_merged[0 : 13 * 365, :, :]
        ...
        reference_periods = np.array([[0, 4 * 365], [5 * 365, 13 * 365]])
    ...
    def compute_bias_from_MODIS(self, data_merged, data_MODIS, lat):
        ...
        data_period = data_merged[5 * 365 : 23 * 365 + 181, :, :]
        data_MODIS_period = data_MODIS[0 : 18 * 365 + 181, :, :]
        ...
        reference_periods = np.array([[0, 8 * 365], [10 * 365, 13 * 365], [16 * 365, 18*365 + 181]])
    ...
    def make_total_bias(self, bias_seawifs, bias_modis, length):
        ...
        date_biases = [4 * 365 + 182, 14 * 365, 19 * 365 + 182]
